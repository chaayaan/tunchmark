...
if (isset($_POST['create_supplier'])) {
    ...
    $conn->query("INSERT INTO suppliers (name, company_name, phone, email, address) VALUES ('$name','$company','$phone','$email','$address')");
    header("Location: suppliers.php?msg=created&page=" . (int)($_GET['page'] ?? 1)); exit;
}
...
if (isset($_POST['update_supplier'])) {
    ...
    $email   = $conn->real_escape_string(trim($_POST['email']));
    $address = $conn->real_escape_string(trim($_POST['address']));
    $conn->query("UPDATE suppliers SET name='$name', company_name='$company', phone='$phone', email='$email', address='$address' WHERE id=$id");
    header("Location: suppliers.php?msg=updated&page=" . (int)($_POST['page'] ?? 1)); exit;
}

// Delete
if (isset($_GET['delete'])) {
    $id    = (int)$_GET['delete'];
    $check = $conn->query("SELECT COUNT(*) FROM stock_in WHERE supplier_id=$id")->fetch_row()[0];
    if ($check > 0) {
        $msg = 'Cannot delete: this supplier has existing stock-in records.';
        $msg_type = 'danger';
    } else {
...
if (isset($_GET['msg'])) {
    $msgs = ['created'=>'Supplier added successfully.','updated'=>'Supplier updated.','deleted'=>'Supplier deleted.'];
    $msg = $msgs[$_GET['msg']] ?? '';
}
...
$count_res   = $conn->query("SELECT COUNT(*) FROM suppliers $where");
$total_rows  = $count_res->fetch_row()[0];
$total_pages = (int)ceil($total_rows / $per_page);

$suppliers = $conn->query("SELECT * FROM suppliers $where ORDER BY name LIMIT $per_page OFFSET $offset");

function supPagUrl($p) {
    $q = $_GET; $q['page'] = $p;
...
<style>
  ...
  .tb-ico { width:32px; height:32px; background:#ecfdf5; border-radius:8px; display:flex; align-items:center; justify-content:center; color:#059669; font-size:13px; flex-shrink:0; }
  ...
  /* Avatar */
  .avatar { width:36px; height:36px; border-radius:50%; background:linear-gradient(135deg,#0ead69,#06795c); color:#fff; display:flex; align-items:center; justify-content:center; font-weight:700; font-size:.85rem; flex-shrink:0; }
...
<div class="page-shell">
  ...
  <header class="top-bar">
    <div class="tb-ico"><i class="bi bi-truck"></i></div>
    <div>
      <div class="tb-title">Suppliers</div>
      <div class="tb-sub">Vendor records</div>
    </div>
    <div class="tb-right">
      <form method="GET" class="search-form">
        ...
        <?php if($search): ?>
          <a href="suppliers.php" style="height:32px;display:inline-flex;align-items:center;padding:0 10px;border:1.5px solid #e4e7ec;border-radius:6px;font-size:.82rem;color:#6b7280;text-decoration:none;">Clear</a>
        <?php endif; ?>
      </form>
      <div class="page-badge">
        p.<?= $page ?>/<?= max(1,$total_pages) ?> &nbsp;·&nbsp; <?= $total_rows ?> suppliers
      </div>
      <button class="tb-btn tb-btn-dark" data-bs-toggle="modal" data-bs-target="#supplierModal" onclick="openAddModal()">
  ...
  <div class="main">
    ...
    <div class="sec">
      <div class="sec-head">
        <span class="sec-ico i-green"><i class="bi bi-truck"></i></span>
        <span class="sec-title">All Suppliers</span>
        <span class="sec-meta">Showing <?= $suppliers->num_rows ?> of <?= $total_rows ?></span>
      </div>

      <?php if($suppliers->num_rows === 0): ?>
        <div style="padding:56px 24px;text-align:center;color:#9ca3af;">
          <i class="bi bi-truck" style="font-size:2.8rem;display:block;margin-bottom:12px;"></i>
          <div style="font-weight:700;font-size:.9375rem;">No suppliers found</div>
          ...
          <?php $suppliers->data_seek(0); while($s = $suppliers->fetch_assoc()):
            $history_res = $conn->query("
                SELECT si.id, si.stock_date, si.remarks,
                       ...
                       SUM(pi.status='in_stock') AS still_in_stock,
                       SUM(pi.status='sold') AS sold
                FROM stock_in si
                LEFT JOIN product_items pi ON pi.stock_in_id = si.id
                WHERE si.supplier_id = {$s['id']}
                GROUP BY si.id
                ORDER BY si.stock_date DESC
            ");
            $batch_count = $history_res->num_rows;
          ?>
          <tr>
            <td style="text-align:center;">
              <button class="expand-btn" onclick="toggleHistory(<?= $s['id'] ?>)" title="View supply history">
                <i class="bi bi-chevron-right" id="chevron-<?= $s['id'] ?>"></i>
              </button>
            </td>
            <td>
              <div style="display:flex;align-items:center;gap:10px;">
                ...
                <div>
                  ...
                  <?php if($s['company_name']): ?><div style="font-size:.75rem;color:#9ca3af;"><?= htmlspecialchars($s['company_name']) ?></div><?php endif; ?>
                  <?php if($s['address']): ?><div style="font-size:.72rem;color:#9ca3af;"><i class="bi bi-geo-alt me-1"></i><?= htmlspecialchars($s['address']) ?></div><?php endif; ?>
                </div>
              </div>
            </td>
            <td style="font-size:.82rem;color:#374151;font-family:monospace;"><?= $s['phone'] ? htmlspecialchars($s['phone']) : '<span style="color:#9ca3af;">—</span>' ?></td>
            <td style="font-size:.82rem;color:#374151;"><?= $s['email'] ? htmlspecialchars($s['email']) : '<span style="color:#9ca3af;">—</span>' ?></td>
            <td style="text-align:center;">
              <?php if($batch_count > 0): ?>
                <span style="background:#e0f2fe;color:#0369a1;font-size:.7rem;font-weight:700;padding:3px 9px;border-radius:12px;"><?= $batch_count ?> batch<?= $batch_count!=1?'es':'' ?></span>
              <?php else: ?>
              ...
              <div style="display:flex;gap:6px;justify-content:center;">
                <button type="button" class="btn-action btn-edit"
                  onclick="openEditModal(
                    <?= $s['id'] ?>,
                    <?= htmlspecialchars(json_encode($s['name'])) ?>,
                    <?= htmlspecialchars(json_encode($s['company_name'] ?? '')) ?>,
                    <?= htmlspecialchars(json_encode($s['phone'] ?? '')) ?>,
                    <?= htmlspecialchars(json_encode($s['email'] ?? '')) ?>,
                    <?= htmlspecialchars(json_encode($s['address'] ?? '')) ?>,
                    <?= $page ?>
                  )" title="Edit"><i class="bi bi-pencil"></i></button>
                <a href="suppliers.php?delete=<?= $s['id'] ?>&page=<?= $page ?><?= $search ? '&search='.urlencode($search) : '' ?>"
                   class="btn-action btn-del"
                   onclick="return confirm('Delete this supplier?')" title="Delete"><i class="bi bi-trash"></i></a>
              </div>
            </td>
          </tr>
          <!-- Expandable history row -->
          <tr class="expand-row" id="history-<?= $s['id'] ?>">
            <td></td>
            <td colspan="5" style="padding:0;background:#fafbfc;">
              <?php if($batch_count === 0): ?>
                <div style="padding:14px 16px;color:#9ca3af;font-size:.82rem;"><i class="bi bi-truck me-1"></i>No supply history yet.</div>
              <?php else: ?>
              <table class="history-tbl">
                ...
                <tbody>
                <?php $history_res->data_seek(0); while($row = $history_res->fetch_assoc()): ?>
                <tr>
                  <td style="font-family:monospace;font-size:.78rem;color:#6b7280;">#<?= $row['id'] ?></td>
                  <td style="font-size:.82rem;"><?= date('d M Y', strtotime($row['stock_date'])) ?></td>
                  <td style="text-align:center;">
                    <span style="background:#e0f2fe;color:#0369a1;font-size:.7rem;font-weight:700;padding:2px 8px;border-radius:10px;"><?= $row['item_count'] ?></span>
                  </td>
                  <td style="text-align:center;"><span class="badge-status badge-in_stock"><?= $row['still_in_stock'] ?></span></td>
                  <td style="text-align:center;"><span class="badge-status badge-sold"><?= $row['sold'] ?></span></td>
                  <td style="font-size:.78rem;color:#9ca3af;"><?= $row['remarks'] ? htmlspecialchars($row['remarks']) : '—' ?></td>
                  <td>
                    <a href="stock_in.php" style="display:inline-flex;align-items:center;justify-content:center;width:26px;height:26px;border:1.5px solid #e4e7ec;border-radius:6px;color:#6b7280;font-size:11px;text-decoration:none;" title="View stock-in">
      ...
      <div class="pag-wrap">
        <?php if($page > 1): ?>
          <a href="<?= supPagUrl(1) ?>" class="pag-btn"><i class="bi bi-chevron-double-left" style="font-size:.65rem;"></i></a>
          <a href="<?= supPagUrl($page-1) ?>" class="pag-btn"><i class="bi bi-chevron-left" style="font-size:.65rem;"></i></a>
        <?php else: ?>
        ...
        <?php for($i=max(1,$page-2); $i<=min($total_pages,$page+2); $i++): ?>
          <a href="<?= supPagUrl($i) ?>" class="pag-btn <?= $i===$page?'active':'' ?>"><?= $i ?></a>
        <?php endfor; ?>
        <?php if($page < $total_pages): ?>
          <a href="<?= supPagUrl($page+1) ?>" class="pag-btn"><i class="bi bi-chevron-right" style="font-size:.65rem;"></i></a>
          <a href="<?= supPagUrl($total_pages) ?>" class="pag-btn"><i class="bi bi-chevron-double-right" style="font-size:.65rem;"></i></a>
        <?php else: ?>
      ...
      <form method="POST">
        ...
        <div class="modal-header" style="background:#1a1a2e;border:none;">
          <h5 class="modal-title" style="color:#fff;font-size:.95rem;font-weight:700;">
            <i class="bi bi-truck me-2"></i><span id="modal_title_text">Add New Supplier</span>
          </h5>
...
function openAddModal() {
  document.getElementById('modal_mode').name = 'create_supplier';
  document.getElementById('modal_id').value = '';
  ['name','company','phone','email','address'].forEach(f => document.getElementById('modal_'+f).value = '');
  document.getElementById('modal_title_text').textContent = 'Add New Supplier';
  document.getElementById('modal_submit_btn').innerHTML = '<i class="bi bi-check-lg me-1"></i>Add Supplier';
}

function openEditModal(id, name, company, phone, email, address, page) {
  document.getElementById('modal_mode').name = 'update_supplier';
  ...
  document.getElementById('modal_title_text').textContent = 'Edit Supplier';
  document.getElementById('modal_submit_btn').innerHTML = '<i class="bi bi-check-lg me-1"></i>Update Supplier';
