...
// Generate filter description for print
function getFilterDescription() {
    $filters = [];
    
    if (!empty($_GET['order_id'])) {
        $filters[] = "Order ID: #" . htmlspecialchars($_GET['order_id']);
    }
    
    if (!empty($_GET['from_date']) && !empty($_GET['to_date'])) {
        $filters[] = "Date Range: " . htmlspecialchars($_GET['from_date']) . " to " . htmlspecialchars($_GET['to_date']);
    } elseif (!empty($_GET['year'])) {
        $filters[] = "Year: " . htmlspecialchars($_GET['year']);
    }
    ...
    if (!empty($_GET['service'])) {
        ...
        foreach ($servicesList as $svc) {
            if ($svc['id'] == $_GET['service']) {
                $filters[] = "Service: " . htmlspecialchars($svc['name']);
                break;
            }
        }
    }
    
    return !empty($filters) ? implode(" | ", $filters) : "All Records";
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        ...
        .stat-green .stat-icon-wrapper { color: #2e7d32; }
        .stat-green .stat-value { color: #1b5e20; }
        
        .stat-orange { 
            background: linear-gradient(135deg, #fff3e0 0%, #ffe0b2 100%);
        }
        .stat-orange .stat-icon-wrapper { color: #ef6c00; }
        .stat-orange .stat-value { color: #e65100; }
        
        .stat-red { 
            background: linear-gradient(135deg, #ffebee 0%, #ffcdd2 100%);
        }
        .stat-red .stat-icon-wrapper { color: #c62828; }
        .stat-red .stat-value { color: #b71c1c; }
        
        .stat-blue { 
            background: linear-gradient(135deg, #e3f2fd 0%, #bbdefb 100%);
        }
        .stat-blue .stat-icon-wrapper { color: #1565c0; }
        .stat-blue .stat-value { color: #0d47a1; }
        ...
        @media print { 
            ...
            .print-service-table tbody tr {
                border: 2px solid #000 !important;
                outline: 2px solid #000 !important;
                outline-offset: -2px;
            }
            
            .print-service-table tbody tr td {
            ...
            .print-service-table tfoot tr {
                border: 2px solid #000 !important;
            }
            
            .print-service-table tfoot td {
                font-weight: bold;
            ...
            .print-orders-table thead tr {
                border: 2px solid #000 !important;
            }
            
            .print-orders-table thead tr th {
            ...
            .print-orders-table tbody tr td {
                border-left: 1px solid #666 !important;
                border-right: 1px solid #666 !important;
                border-top: 0.5px solid #ccc !important;
                border-bottom: 0.5px solid #ccc !important;
            ...
            .print-orders-table .items-list li {
                margin-bottom: 0.2px;
                padding: 0.2px 0;
            }
            ...
            /* Ultra compact spacing for maximum rows */
            tr {
                line-height: 0.85;
            }
            
            /* Force visible borders on every element */
            .print-orders-table tr,
            .print-service-table tr {
                outline: 1.5px solid #000 !important;
            }
            
            /* Double ensure cell borders */
            td, th {
...
<div class="container-fluid p-4">
    ...
    <div class="page-header d-flex justify-content-between align-items-center">
        ...
        <div>
            ...
            <button onclick="printFilteredReport()" class="btn btn-sm btn-success no-print me-2">
        ...
        <div class="col-6 col-md-3">
            <div class="modern-stat-card stat-green">
                <div class="card-body p-3">
                    ...
                    <div class="stat-sublabel">৳<?= number_format($totalPaid, 0) ?></div>
                </div>
            </div>
        </div>
        ...
        <div class="col-6 col-md-3">
            <div class="modern-stat-card stat-orange">
                <div class="card-body p-3">
                    ...
                    <div class="stat-sublabel">৳<?= number_format($totalPending, 0) ?></div>
                </div>
            </div>
        </div>
        ...
        <div class="col-6 col-md-3">
            <div class="modern-stat-card stat-red">
                <div class="card-body p-3">
                    ...
                    <div class="stat-sublabel">৳<?= number_format($totalCancelled, 0) ?></div>
                </div>
            </div>
        </div>
        ...
        <div class="col-6 col-md-3">
            <div class="modern-stat-card stat-blue">
                <div class="card-body p-3">
                    ...
                    <div class="stat-sublabel">৳<?= number_format($grandTotal, 0) ?></div>
                </div>
            </div>
        </div>
    </div>
...
<div id="printContainer" style="display: none;">
    <div class="print-container">
        ...
        <div class="print-header">
            ...
            <div class="filter-info">
                <strong>Generated:</strong> <?= date('F d, Y h:i A') ?> | <strong>Total Orders:</strong> <?= $total_records ?>
            </div>
        </div>

        <!-- Service Summary Section -->
        <div class="print-service-summary">
            <h4>SERVICE SUMMARY (PAID ORDERS ONLY)</h4>
            <table class="print-service-table">
                <thead>
                    <tr>
                        <th style="width: 50%;">Service Name</th>
                        <th class="text-center" style="width: 20%;">Quantity</th>
                        <th class="text-end" style="width: 30%;">Total Amount (৳)</th>
                    </tr>
                </thead>
                <tbody>
                    <?php 
                    ...
                    ?>
                    <tr>
                        <td><strong><?= htmlspecialchars($serviceName) ?></strong></td>
        ...
        <!-- Orders List Section -->
        <div class="print-orders-section">
            <h4>DETAILED ORDERS LIST</h4>
            <table class="print-orders-table">
                <thead>
                    <tr>
                        <th style="width: 7%;">Order</th>
                        <th style="width: 15%;">Customer</th>
                        <th style="width: 12%;">Phone</th>
                        <th style="width: 44%;">Items & Services</th>
                                ...
                                <?php if (!empty($order['items'])): ?>
                                    <?php foreach ($order['items'] as $idx => $item): ?>
                                        <li>
                                            ...
                                            <?= htmlspecialchars($item['service_name']) ?> 
                                            (<?= number_format((float)$item['quantity'], 0) ?> × ৳<?= number_format((float)$item['unit_price'], 2) ?>)
                                        </li>
                                    <?php endforeach; ?>
                                <?php else: ?>
...
function printFilteredReport() {
    ...
    // Reload the page to restore all event listeners and functionality
    location.reload();
}
</script>
</body>
